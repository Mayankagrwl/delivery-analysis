...
- **Grafana/Loki queries are scoped per env** via the Loki `environment` label (`grafana.loki_environment_value`: `prod` → `"production"`, others as-is; `srm.environment` drives `_Collector.environment_value`). Without this, an `ALL` run returns the same logs/analysis for every env (one Loki holds all envs). `env` is a dashboard-only variable, not a Loki label. Legacy single-host runs where `srm.environment` is unset emit no env label (unchanged).
...
- New workflow `vars.*`: `TEMPO_ID`, `NOTIFICATION_COMPONENT`, `NOTIFICATION_SUCCESS_MARKERS`, `NOTIFICATION_REQUEST_ID_FIELD`, `TRACE_ID_FIELD`, `REQUEST_ID_LOOKBACK_HOURS`.
