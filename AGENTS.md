...
- URL precedence (single env): `--url` (full-URL escape hatch) > `SRM_BASE_URL` env > per-env computed URL. **The override is a single-env escape hatch and is ignored in an `ALL`/multi-env run** (`run_collect_environments` honors `cfg.srm_base_url_override` only when `len(targets) == 1`); otherwise it collapses every env onto one URL → identical data. In `ALL` mode each env always uses `srm_url_for_env(env)`; the resolved URL (via `safe_url`) is printed per env and shown in each env's `summary.md` and the index.
...
- **Grafana/Loki queries are scoped per env** via the Loki `environment` label (`grafana.loki_environment_value`: `prod` → `"production"`, others as-is, overridable via `GRAFANA_ENV_LABEL_VALUES`; `srm.environment` drives `_Collector.environment_value`). The matcher is present on **every** Loki query path — priority queries, per-URN queries, the Notification probe, panel-derived queries (`ensure_environment_label`), and the stats query. Without this, an `ALL` run returns the same logs/analysis for every env (one Loki holds all envs). `env` is a dashboard-only variable, not a Loki label, and `environment` is never put in the SRM URL. Legacy single-host runs where `srm.environment` is unset emit no env label (unchanged).
...
**A `request_id` run always probes Notification + Tempo regardless of verdict** (`STALE`, `FRESH`, `NO_RECORDS`); `SRM_ERROR` still respects `QUERY_GRAFANA_ON_SRM_ERROR`. An id that already completed is normally absent from SUBMITTED/GRANTED (`NO_RECORDS`) — the completion evidence is in the logs, not SRM. Outcome resolution in `collect.py`: notification success ⇒ `infra_success` (STGPT skipped, wins even when also STALE); else if `STALE` ⇒ run `analyze_staleness`; else (not stale, no success) ⇒ `investigate` record with **no STGPT call, 0 tokens**. `AnalysisStatus` includes `infra_success` and `investigate`. The probe window is `now - REQUEST_ID_LOOKBACK_HOURS .. now` (env `REQUEST_ID_LOOKBACK_HOURS`, default 168 = 7 days) so older completed requests are found; `build_time_packs` prepends a `lookback` pack in request_id mode. `SrmResult.request_outcome` (SUCCESS / INVESTIGATE / STALE) surfaces in the per-env summary and the top-level index.

- **Notification success gate (two-hop correlation).** The success markers (`NOTIFICATION_SUCCESS_MARKERS`, default `successfully processed|mail sent to`, case-insensitive) are logged **keyed by a correlation `requestId`, not by the DeliveryRequest URN**, so `grafana.py`'s `_Collector._notification_probe` correlates in two hops on `NOTIFICATION_COMPONENT` (default `notification`): (1) `{component="<notification>"} |= "<urn>"` finds the payload/context line and its `requestId` (field `NOTIFICATION_REQUEST_ID_FIELD`, default `requestId`); (2) `{component="<notification>"} |= "<requestId>"` finds the marker lines. All probe queries have **no level filter**, so DEBUG `"successfully processed"` is never dropped regardless of `INCLUDE_DEBUG_LOGS`. The resolved `requestId` and `notification_lines_scanned` are recorded for diagnosability. On a match, `analyze_staleness` short-circuits: **no STGPT call**, `AnalysisRecord.status="infra_success"`, `fallback_used=False`, `tokens_used=0`, notification lines as citations. No match ⇒ existing persona analysis runs (STALE) or the `investigate` no-AI outcome (not stale).
...
- New workflow `vars.*`: `TEMPO_ID`, `NOTIFICATION_COMPONENT`, `NOTIFICATION_SUCCESS_MARKERS`, `NOTIFICATION_REQUEST_ID_FIELD`, `GRAFANA_ENV_LABEL_VALUES`, `TRACE_ID_FIELD`, `REQUEST_ID_LOOKBACK_HOURS`.
